...
/**
 * Trong ví dụ này, counter là một tham chiếu tới một giá trị số (0).
 * Bạn có thể sử dụng counter.value để truy cập và thay đổi giá trị của tham chiếu.
 * Khi giá trị thay đổi, component sẽ tự động được render lại để phản ánh sự thay đổi.
 */
...
onMounted(async () => {
    ...
    await getDataProductEdit()
})
onBeforeUnmount(() => {
    console.log('Before Unmount Hook loading....');
})
onUnmounted(() => {
    console.log('Unmounted Hook loading...');
})

const getDataProductEdit = async () => {
    let rs = await axios.post(`http://localhost:9898/api/editProduct`, {id: props.id})
    form.value = rs.data.product
}
const getPhoto = () => {
    let photo = ''
    if(form.value.photo && form.value.photo.indexOf('base64') != -1){
        ...
        photo = `http://localhost:9898/upload/${form.value.photo}`
    }
    return photo
}
...
const saveProduct = async () => {
    ...
    axios.post('http://localhost:9898/api/updateProduct', formData)
        .then(rs => {
            console.log('rs', rs)
            form.value.id = ''
            form.value.name = ''
