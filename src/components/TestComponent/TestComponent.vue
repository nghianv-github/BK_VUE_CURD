...
<template>
    ...
    <h1>click me {{ counter }}</h1>

    <h5>{{ info }}</h5>

    <button @click="handleIncrementConter"> + </button>
    <button @click="handleIncrementAddInfo({
        id: 2,
        data: 'NghiaNV'
    })"> + </button>
</template>
...
export default {
    ...
    methods: {
        handleIncrementConter(){
            this.counter = this.counterStore.counter++
        },

        handleIncrementAddInfo(info){
            this.counterStore.increment(info)
            this.info = this.counterStore.userInfo
            this.counter = this.counterStore.counter
        }
    }
};
...
<style scoped>
/* Các quy tắc CSS ở đây sẽ chỉ áp dụng cho component này */
